...
if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM sale_returns_details WHERE id = ?";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $tinvoice = trim($_GET["tinvoice"]);
    $tqty = trim($_GET["tqty"]);
    $qty = trim($_GET["qty"]);

    // Finding the maximim allowable quantity
    $b=array($tinvoice,-$tqty,$qty);
    $max = array_sum($b);
    ...
    if(empty($quantity_err)){
        ...
        //UPDATE sale_returns DETAILS TABLE          
        $sql2 = "UPDATE `sale_returns_details` SET `product`=?,`quantity`=? WHERE id=?;";       
        ...
        <div class="container">
            <div class="row">
                <div class="mx-auto">
                    ...
                    <p>Please fill this form and submit to update the item in the sale_returns.
