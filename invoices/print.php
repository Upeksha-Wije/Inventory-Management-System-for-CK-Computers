    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $s=$customer; 
                $count="SELECT * FROM customers WHERE id=$s"; // SQL to get customer name 
                ...
                $sql2 = "SELECT * FROM invoices_details WHERE invoices_id=$id && status=1 ORDER BY `id` DESC";
                ...
                function Header()
                {
                    ...
                    $this->cell(0,15,'Invoice Details',0,1,'L');
                ...
                $pdf->cell(60,15,'Sales Invoice ID :',0,0,'L');
                ...
                $pdf->SetFont('Arial','B',14);
                $width_cell=array(20,27,65,32,22,24);//190 mm total
                ...
                $pdf->Cell($width_cell[3],10,'Selling Price',1,0,"R",true); // Fourth header column
                ...
                foreach ($link->query($sql2) as $row2) {
                    ...
                    $pdf->Cell($width_cell[3],10,$sell,1,0,"R",$fill);
                    $pdf->Cell($width_cell[4],10,$quantity,1,0,"R",$fill);
                    $pdf->Cell($width_cell[5],10,$subtotal,1,1,"R",$fill);
